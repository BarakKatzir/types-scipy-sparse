...
> __Note__: The version of `uv` is fixed separately in different places as the `uv` command is run separately in different environments (the dev venv, tox venv, when building wheels, an in github workflows).

Here are a few steps for anyone to get started developing the package:
